...
return new Promise((res, rej)=>{
  db.run("INSERT INTO threads(board, text, delete_password, reported) VALUES(?, ?, ?, ?)",[body.board, body.text, body.password, 'false'],(err)=>{
    if(err){
      rej(err)
    }
...
const addReply = (body) => {
  return new Promise((res, rej)=>{
    db.run("INSERT INTO replies(thread_id, text, delete_password, reported) VALUES(?, ?, ?, ?)",[body.id, body.text, body.password, 'false'],(err)=>{
      if(err){
        rej(err)
      }
      db.run("UPDATE threads SET bumped_on = CURRENT_TIMESTAMP WHERE _id = ?",[body.id],(err)=>{
        if(err){
          rej(err)
        }
        res("Data added successfully")
...
const deleteThread = (id, pw) => {
  return new Promise((res, rej)=>{
    db.get("SELECT delete_password FROM threads WHERE _id = ?", [id], (err, row)=>{
      if(err){
        rej(err)
      }
...
const deleteReply = (id, pw) => {
  return new Promise((res, rej)=>{
    db.get("SELECT delete_password FROM replies WHERE _id = ?", [id], (err, row)=>{
      if(err){
        rej(err)
      }
      if(row.delete_password == pw ){
        db.run("UPDATE replies SET text = '[deleted]' WHERE _id = ?",[id],(err)=>{
          if(err){
            rej(err)
          }
          res("success")
...
const getThread = (id) => {
  return new Promise((res, rej)=> {
    db.get("SELECT _id, text, created_on, bumped_on FROM threads WHERE _id = ?", [id], async(err, row)=>{
       if(err){
        rej(err)
       }
...
const getThreads = (board) => {
  return new Promise((res, rej)=> {
    db.all("SELECT _id, text, created_on, bumped_on FROM threads WHERE board = ? ORDER BY bumped_on DESC LIMIT 10", [board], (err, rows)=>{
       if(err){
        rej(err)
       }
       const newRows = Promise.all(rows.map(async(ele)=>{
...
const getReplies = (id, limit) => {
  return new Promise((res, rej)=> {
    db.all("SELECT _id, text, created_on FROM replies WHERE thread_id = ? ORDER BY created_on DESC LIMIT ?", [id, limit], (err, rows)=>{
       if(err){
        rej(err)
       }
       res(rows)
...
const reportThread = (id) => {
  return new Promise((res, rej)=>{
    db.run("UPDATE threads SET reported = 'true' WHERE _id = ?", [id], (err)=>{
      if(err){
        rej(err)
      }
      res('reported')
...
const reportReply = (id) => {
  return new Promise((res, rej)=>{
    db.run("UPDATE replies SET reported = 'true' WHERE _id = ?", [id], (err)=>{
      if(err){
        rej(err)
      }
      res('reported')
